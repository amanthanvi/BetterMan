...
export const UltimateDocumentViewer: React.FC<DocumentViewerProps> = ({
	...
	useEffect(() => {
		...
		const observer = new IntersectionObserver(
			(entries: IntersectionObserverEntry[]) => {
				...
				if (
					maxVisibleEntry &&
					maxVisibleEntry.target instanceof HTMLElement
				) {
	...
	return (
		...
		>
			...
			{/* Table of Contents Overlay - when TOC is open */}
			<AnimatePresence>
				{showToc && (
					<motion.div
						initial={{ opacity: 0 }}
			...
			{/* Table of Contents Sidebar - Improved positioning */}
			...
			<div className="ultimate-content-wrapper">
				<div
					className={cn(
