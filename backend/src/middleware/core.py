...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        super().__init__(app)
        self.sensitive_paths = {"/auth/login", "/auth/register", "/auth/change-password"}
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        logger.info(
            ...
            extra={
                ...
                "query_params": dict(request.query_params) if request.url.path not in self.sensitive_paths else "[REDACTED]",
                "client_host": self._hash_ip(request.client.host) if request.client else None,
...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # Don't compress if already compressed or small response
        if (
            "content-encoding" in response.headers or
            int(response.headers.get("content-length", 0)) < 1024  # 1KB
        ):
            return response
...
def setup_middleware(app: ASGIApp) -> None:
    ...
    app.add_middleware(
        ...
        allow_headers=settings.CORS_HEADERS.split(",") + ["X-CSRF-Token", "X-Request-ID"],
